...
export default function PrskanjeAdd({isOpen, handleClose, id,updated, setUpdated}) {
    console.log(isOpen)
    const [naziv, setNaziv]= useState("");
    const [opis, setOpis]= useState("");
    const [parcela, setParcela]= useState("");
    const [sredstvo, setStedstvo]= useState("");
    ...
    const [datum, setDatum] =useState(new Date());
     ...
     async function handleSubmit(e){
       ...
       try {
        ...
        const response= await axios.patch(`http://localhost:5000/api/data/prskanja/${id}`,prskanje );
  ...
  <div className="form-group">
    ...
    <input type="text" className="form-control" id="sredstvo" placeholder="Sredstvo" value ={sredstvo} onChange={(e)=>{setStedstvo(e.target.value)}}/>
  </div>
  <div className="form-group">
